...
#include "brave/browser/ui/views/ipfs/ipfs_info_bubble_view.h"
...
// IpfsBubbleHeaderView is the UI element (view) that represents the header of a
// IpfsInfoBubbleView. The header shows the status of the site's identity check
// and the name of the site's identity.
class IpfsBubbleHeaderView : public views::View {
 public:
  IpfsBubbleHeaderView(IpfsInfoBubbleView* bubble, int side_margin);
  ~IpfsBubbleHeaderView() override;
  ...
  void AddEvCertificateDetailsLabel(
      const IpfsInfoBubbleView::IdentityInfo& identity_info);
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(IpfsBubbleHeaderView);
};
...
IpfsBubbleHeaderView::IpfsBubbleHeaderView(IpfsInfoBubbleView* bubble,
                                           int side_margin)
    : bubble_(bubble) {
...
IpfsBubbleHeaderView::~IpfsBubbleHeaderView() = default;

void IpfsBubbleHeaderView::SetDetails(const base::string16& details_text) {
  ...
  views::StyledLabel::RangeStyleInfo link_style =
      views::StyledLabel::RangeStyleInfo::CreateForLink(
          base::BindRepeating(&IpfsInfoBubbleView::SecurityDetailsClicked,
                              base::Unretained(bubble_)));

  security_details_label_->AddStyleRange(details_range, link_style);
}

void IpfsBubbleHeaderView::AddResetDecisionsLabel() {
  ...
  views::StyledLabel::RangeStyleInfo link_style =
      views::StyledLabel::RangeStyleInfo::CreateForLink(
          base::BindRepeating(&IpfsInfoBubbleView::ResetDecisionsClicked,
                              base::Unretained(bubble_)));
...
void IpfsBubbleHeaderView::AddPasswordReuseButtons(
    bool is_saved_password,
    views::Button::PressedCallback change_password_callback,
    views::Button::PressedCallback password_reuse_callback) {
  ...
  if (change_password_template) {
    ...
    change_password_button->SetID(
        IpfsInfoBubbleView::VIEW_ID_PAGE_INFO_BUTTON_CHANGE_PASSWORD);
  }
  ...
  allowlist_password_reuse_button->SetID(
      IpfsInfoBubbleView::VIEW_ID_PAGE_INFO_BUTTON_ALLOWLIST_PASSWORD_REUSE);
...
IpfsInfoBubbleView::~IpfsInfoBubbleView() {}

// static
views::BubbleDialogDelegateView* IpfsInfoBubbleView::CreateIpfsInfoBubble(
  ...
  return new IpfsInfoBubbleView(anchor_view, anchor_rect, parent_view, profile,
                                web_contents, url, std::move(closing_callback));
}

void IpfsInfoBubbleView::SecurityDetailsClicked(const ui::Event& event) {
...
void IpfsInfoBubbleView::ResetDecisionsClicked() {
...
IpfsInfoBubbleView::IpfsInfoBubbleView(
  ...
  header_ = layout->AddView(
      std::make_unique<IpfsBubbleHeaderView>(this, side_margin));
  ...
  if (!profile->IsGuestSession()) {
    ...
    site_settings_link = layout->AddView(std::make_unique<PageInfoHoverButton>(
        ...
        IpfsInfoBubbleView::VIEW_ID_PAGE_INFO_LINK_OR_BUTTON_SITE_SETTINGS,
...
void IpfsInfoBubbleView::WebContentsDestroyed() {
  weak_factory_.InvalidateWeakPtrs();
}

void IpfsInfoBubbleView::OnPermissionChanged(
...
gfx::Size IpfsInfoBubbleView::CalculatePreferredSize() const {
...
void IpfsInfoBubbleView::SetCookieInfo(const CookieInfoList& cookie_info_list) {
...
void IpfsInfoBubbleView::SetPermissionInfo(
...
void IpfsInfoBubbleView::SetIdentityInfo(const IdentityInfo& identity_info) {
...
void IpfsInfoBubbleView::SetPageFeatureInfo(const PageFeatureInfo& info) {
#if defined(OS_WIN) && BUILDFLAG(ENABLE_VR)
...
void IpfsInfoBubbleView::LayoutPermissionsLikeUiRow(views::GridLayout* layout,
                                                    bool is_list_empty,
                                                    int column_id) {
  ChromeLayoutProvider* layout_provider = ChromeLayoutProvider::Get();
  if (is_list_empty) {
...
void IpfsInfoBubbleView::DidChangeVisibleSecurityState() {
  presenter_->UpdateSecurityState();
}

#if BUILDFLAG(FULL_SAFE_BROWSING)
std::unique_ptr<PageInfoUI::SecurityDescription>
IpfsInfoBubbleView::CreateSecurityDescriptionForPasswordReuse() const {
  std::unique_ptr<PageInfoUI::SecurityDescription> security_description(
...
std::unique_ptr<views::View> IpfsInfoBubbleView::CreateSiteSettingsView() {
...
void IpfsInfoBubbleView::HandleMoreInfoRequest(views::View* source) {
...
void ShowIpfsInfoDialogImpl(Browser* browser,
                            content::WebContents* web_contents,
  ...
  views::BubbleDialogDelegateView* bubble =
      IpfsInfoBubbleView::CreateIpfsInfoBubble(
